...
## Overview
`ai_service` is a FastAPI web service that exposes the `ai_api.AIInterface` via REST endpoints. It wraps AI implementations (like `openai_impl`) and makes them accessible over HTTP, enabling distributed AI processing.

## Purpose

- **REST API**: Exposes AI functionality via HTTP endpoints
- **Service Layer**: Enables distributed architecture where AI runs on separate infrastructure
- **OpenAPI Spec**: Auto-generates API documentation and client code
...
```
HTTP Request → FastAPI (ai_service)
    ↓
ai_api.get_client()
    ↓
...
result = client.generate_response(
    ...
    system_prompt="Extract the action.",
...
The service automatically loads environment variables from `.env` using `python-dotenv`.
...
The service can be deployed to any cloud platform that supports Docker containers:
- AWS ECS/Fargate
- Google Cloud Run
...
The `ai_service_client` package is auto-generated from the OpenAPI spec:

```bash
# Generate client from running service
...
```python
logger.info("Processing request: %s", request.user_input)
logger.error("Failed to generate response: %s", error)
```

## Design Principles

1. **Separation of Concerns**: Service layer separate from business logic
2. **Dependency Injection**: Uses registered AI implementation
3. **RESTful Design**: Standard HTTP methods and status codes
4. **OpenAPI Standard**: Auto-generated documentation and clients
5. **12-Factor App**: Configuration via environment, stateless, scalable
